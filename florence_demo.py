...
# Temporarily disable logging to prevent JSON serialization issues during startup
logging.getLogger("transformers").setLevel(logging.ERROR)
warnings.filterwarnings("ignore", category=UserWarning, message=".*resume_download.*")
warnings.filterwarnings("ignore", category=FutureWarning, message=".*resume_download.*")

# Import subprocess here since we need it
import subprocess

# Create lazy loading for models and processors to avoid JSON serialization during import
class LazyLoader:
    def __init__(self):
        self._models = {}
        self._processors = {}
        self._loaded_models = set()
        self._loaded_processors = set()
    ...
    def _load_model(self, model_name):
        ...
        try:
            # Try to install flash_attn if needed
            ...
            try:
                ...
                try:
                    ...
                    print(f"Warning: Could not install flash_attn. Loading {model_name} without it...")
            ...
            # Move to CUDA if available, else keep on CPU
            import torch
    ...
    def _load_processor(self, model_name):
        """Internal method to load a specific processor"""
        try:
            processor = AutoProcessor.from_pretrained(model_name, trust_remote_code=True)
            return processor
        except Exception as e:
            print(f"Error loading processor for {model_name}: {e}")
            raise e
    ...
    def get_model(self, model_name):
        ...
        if model_name not in self._models:
            ...
            # Temporarily suppress logging to avoid JSON serialization issues
    ...
    def get_processor(self, model_name):
        """Get a processor, loading it if necessary"""
        if model_name not in self._processors:
            ...
            try:
                self._processors[model_name] = self._load_processor(model_name)
            finally:
                # Restore original logging level
                transformers_logger.setLevel(original_level)
...
# Define functions to access models and processors
def get_model(model_name):
    return loader.get_model(model_name)

def get_processor(model_name):
    return loader.get_processor(model_name)

# Replace direct access with lazy loading functions
def get_models_list():
    return [
        'microsoft/Florence-2-large-ft',
        'microsoft/Florence-2-large', 
    ...
    with gr.Tab(label="Florence-2 Image Captioning"):
        with gr.Row():
            with gr.Column():
                ...
                model_selector = gr.Dropdown(choices=get_models_list(), label="Model", value='microsoft/Florence-2-large')
        ...
        # Commenting out examples that reference non-existent image files
        # gr.Examples(
        #     examples=[
