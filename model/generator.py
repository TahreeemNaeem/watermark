...
class SkipEncoderDecoder(nn.Module):
    def __init__(self, input_depth, num_channels_down = [128] * 5, num_channels_up = [128] * 5, num_channels_skip = [128] * 5, skip_connection=True):
        ...
        for i in range(len(num_channels_down)):
            ...
            if num_channels_skip[i] != 0 and self.skip_connection:
                model_tmp.add_module(str(len(model_tmp) + 1), Concat(1, skip, deeper))
            else:
                model_tmp.add_module(str(len(model_tmp) + 1), deeper)
            
            if self.skip_connection and num_channels_skip[i] != 0:
                out_ch = num_channels_skip[i] + (num_channels_up[i + 1] if i < (len(num_channels_down) - 1) else num_channels_down[i])
            else:
                out_ch = (num_channels_up[i + 1] if i < (len(num_channels_down) - 1) else num_channels_down[i])
                
            model_tmp.add_module(str(len(model_tmp) + 1), nn.BatchNorm2d(out_ch))
            ...
            deeper.add_module(str(len(deeper) + 1), nn.Upsample(scale_factor = 2, mode = 'bilinear', align_corners=False))

            if self.skip_connection and num_channels_skip[i] != 0:
                model_tmp.add_module(str(len(model_tmp) + 1), Conv2dBlock(num_channels_skip[i] + k, num_channels_up[i], 3, 1, bias = False))
            else:
                model_tmp.add_module(str(len(model_tmp) + 1), Conv2dBlock(k, num_channels_up[i], 3, 1, bias = False))
        ...
        self.model.add_module(str(len(self.model) + 1), nn.Tanh())  # Changed from Sigmoid to Tanh for better range
...
def input_noise(INPUT_DEPTH, spatial_size, scale = 1./10):
    ...
    return torch.randn(*shape) * scale  # Changed to normal distribution for better initialization